...
pub fn map_subxt_err(e: subxt::GenericError<std::convert::Infallible>) -> actix_web::Error {
    ...
    //     _ => json!(e.to_string()),
    // };
    let json_err = json!("temp error msg");
    let e = format!("{:#?}", "");
...
pub fn map_sf_err(e: subxt::GenericError<subxt::RuntimeError<sugarfunge::runtime_types::sp_runtime::DispatchError>>) -> actix_web::Error {
    let json_err = json!("temp error msg");
    let e = format!("{:#?}", "");
    let req_error = RequestError {
        message: json_err,
