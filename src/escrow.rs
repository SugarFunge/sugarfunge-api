...
pub async fn register(
    ...
    let result = api
        .tx()
        .escrow()
        .register_escrow(req.class_id, metadata)
...
fn transform_owners_input(
    in_owners: Vec<String>,
) -> Vec<AccountId32> {
    in_owners
        .into_iter()
        .map(|current_owner| 
...
fn transform_owners_output(
    in_owners: Vec<AccountId32>,
) -> Vec<String> {
    in_owners
        .into_iter()
        .map(|current_owner| 
...
pub async fn create_escrow(
    ...
    let result = api
        .tx()
        .escrow()
        .create_account(req.class_id, owners, req.shares.clone())
        .sign_and_submit_then_watch(&signer)
        .await
        .map_err(map_subxt_err)?
        .wait_for_finalized_success()
...
pub async fn deposit_assets(
    ...
    let result = api
        .tx()
        .escrow()
        .deposit_assets(escrow, req.class_ids.clone(), req.asset_ids.clone(), req.amounts.clone())
        .sign_and_submit_then_watch(&signer)
        .await
        .map_err(map_subxt_err)?
        .wait_for_finalized_success()
